...
class DG_model(nn.Module):
    ...
    def forward(self, input, norm_flag):
        ...
        features = self.embedder(features, norm_flag)

        ##clean feature
        clean_features_no_norm=features
        ...
        noise_features = self.augment(features)


        noise_features_norm = torch.norm(noise_features, p=2, dim=2, keepdim=True).clamp(min=1e-12) ** 0.5 * (2) ** 0.5
        ...
        return aug_classifier_out, aug_features, clean_classifier_out, clean_features
