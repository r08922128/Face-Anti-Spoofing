...
def prepare_noise_features_labels(args, real_noise_features, fake_noise_features, fake_session_labels):
	...
	fake_session_labels = fake_session_labels.unsqueeze(-1).expand(-1, args.h_degree).reshape(-1, 1)
...
def prepare_features_labels(real_features, fake_features, fake_session_labels):
	...
	fake_session_labels = fake_session_labels
	...
	if args.mode == 'train':
		...
		for epoch in range(args.load + 1, args.epochs):
			...
			for index, ((real_images, real_spoof_labels, real_session_labels, real_folder_name), (fake_images, fake_spoof_labels, fake_session_labels, fake_folder_name)) in enumerate(tqdm(zip(train_real_data, train_fake_data), total = train_data_len, ncols = 70, desc = 'Training')):
				...
				total_features, total_labels = prepare_features_labels(real_features, fake_features, fake_spoof_labels)

				real_noise_features = noise_features[:real_images.shape[0]]
				fake_noise_features = noise_features[real_images.shape[0]:]
				total_noise_features, total_noise_labels = prepare_noise_features_labels(args, real_noise_features, fake_noise_features, fake_spoof_labels)
				...
				total_loss = classification_loss + 10 * real_domain_loss + 2 * gen_asymmetricTripletLoss + aug_asymmetricTripletLoss + reconLoss
...
def test(args, device):
	...
	with torch.no_grad():
		...
		cal_AUC(pred_list, label_list, folder_name_list, './test_pred' + ('_S.csv' if args.test_data_path == '../siw_test' else '_O.csv'), False)
