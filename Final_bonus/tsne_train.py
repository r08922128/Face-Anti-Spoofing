...
def train(args, device):
    ...
    ENet.load_state_dict(torch.load(join(save_path, 'E_SSDG_new_49.pth')))
    ...
    for i in range(all_realfake_label.size(0)):
        ...
        elif all_realfake_label[i].item()==1 and all_session_label[i].item()==1:
            realfake_session_label.append(3)
        elif all_realfake_label[i].item()==2 and all_session_label[i].item()==0:
            realfake_session_label.append(4)
        elif all_realfake_label[i].item()==2 and all_session_label[i].item()==1:
            realfake_session_label.append(5)
    ...
    c=['#52C0D4','#F6C75C','#1D99A6','#DDA230', '#0A5F63', '#9B6D25']
    ...
    plt.legend([p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11],['real session1','real session2','print session1','print session2','replay session1','replay session2','hallu real s1','hallu real s2','hallu print s1','hallu print s2','hallu replay s1','hallu replay s2'],scatterpoints=1)
