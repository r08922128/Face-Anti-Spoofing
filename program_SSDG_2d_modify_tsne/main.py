...
def train(args, device):
	...
	if args.mode == 'train':
		...
		if args.load != -1:
			ENet.load_state_dict(torch.load(join(save_path, 'E_SSDG_new_99.55555555555556_0.9999614197530864_19.pth')))
			DNet.load_state_dict(torch.load(join(save_path, 'D_SSDG_new_99.55555555555556_0.9999614197530864_19.pth')))
		...
		for epoch in range(args.load + 1, args.epochs):
			...
			for index, ((real_images, real_spoof_labels, real_session_labels, real_folder_name), (fake_images, fake_spoof_labels, fake_session_labels, fake_folder_name)) in enumerate(tqdm(zip(train_real_data, train_fake_data), total = train_data_len, ncols = 70, desc = 'Training')):
				...
				reconLoss = MSELoss(aug_features_no_norm, clean_features_no_norm) - torch.mean(cosine_similarity(aug_features_no_norm, clean_features_no_norm) - 1, dim = 0)

				#DNet
				real_features = aug_features_no_norm[:real_images.shape[0]*(args.h_degree + 1)]
				fake_features = aug_features_no_norm[real_images.shape[0]*(args.h_degree + 1):]
...
def test(args, device):
	...
	with torch.no_grad():
		...
		for index, (images, labels, sessions, folder_name) in enumerate(tqdm(test_data, ncols = 70)):
			...
			pred_list.append(aug_predict[0].view(1,-1))
			label_list.append(labels[0].view(1,-1))
			folder_name_list.append(folder_name[0])
